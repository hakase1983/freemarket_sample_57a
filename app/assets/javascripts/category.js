$(function(){
  ...
  function appendChidrenBox(insertHTML){
    ...
    childSelectHtml = `<div class='select-wrap' id= 'children_wrapper'>
                        ...
                        <select class="select-default" id="child_category" name="item[category_id]">
                          <option >---</option>
                          ${insertHTML}
                        <select>
                      </div>`;
  ...
  function appendEditChidrenBox(insertHTML){
    ...
    childSelectHtml = `<div class='select-wrap' id= 'children_wrapper'>
                        ...
                        <select class="select-default" id="edit-child_category" name="item[category_id]">
                          <option >---</option>
                          ${insertHTML}
                        <select>
                      </div>`;
...
$('#parent_category').on('change', function(){
  var parentCategory = $('option:selected').val(); //選択された親カテゴリーの名前を取得
...
$('#edit-parent_category').on('change', function(){
  ...
  if (parentCategory != "---"){ //親カテゴリーが初期値でないことを確認
    $.ajax({
      url: 'edit/get_category_children',
      type: 'GET',
      data: { parent_name: parentCategory },
      dataType: 'json'
    })
...
$('.category').on('change', '#edit-child_category', function(){
  var childId = $('#edit-child_category option:selected').data('category'); //選択された子カテゴリーのidを取得
  if (childId != "---"){ //子カテゴリーが初期値でないことを確認
    $.ajax({
      url: 'edit/get_category_grandchildren',
      type: 'GET',
      data: { child_id: childId },
      dataType: 'json'
    })
